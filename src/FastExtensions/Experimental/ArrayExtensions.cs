...
internal static class ImmutableArrayExtensions
{
    ...
    {
        return items != null ? ImmutableArray.CreateRange<T>(items) : ImmutableArray<T>.Empty;
    }
    ...
    {
        return items != null ? ImmutableArray.CreateRange<T>(items) : default;
    }
    ...
    public static ImmutableArray<TResult> SelectAsArray<TItem, TResult>(this ImmutableArray<TItem> array, Func<TItem, bool> predicate, Func<TItem, TResult> selector)
    {
        ...
        var builder = ImmutableArray.CreateBuilder<TResult>(array.Length);
    ...
    public static async ValueTask<ImmutableArray<TResult>> SelectAsArrayAsync<TItem, TResult>(this ImmutableArray<TItem> array, Func<TItem, CancellationToken, ValueTask<TResult>> selector, CancellationToken cancellationToken)
    {
        ...
        foreach (var item in array)
        {
            // Shouldn't this be checking for IsCompleted?
    ...
    public static ImmutableArray<TResult> ZipAsArray<T1, T2, TResult>(this ImmutableArray<T1> self, ImmutableArray<T2> other, Func<T1, T2, TResult> map)
    {
        ...
        switch (self.Length)
        {
            ...
            default:
                var builder = ImmutableArray.CreateBuilder<TResult>(self.Length);
    ...
    {
        var builder = ImmutableArray.CreateBuilder<T>(first.Length + second.Length + third.Length);
    ...
    {
        var builder = ImmutableArray.CreateBuilder<T>(first.Length + second.Length + third.Length + fourth.Length);
        builder.AddRange(first);
        builder.AddRange(second);
        builder.AddRange(third);
    ...
    {
        var builder = ImmutableArray.CreateBuilder<T>(first.Length + second.Length + third.Length + fourth.Length + fifth.Length);
        builder.AddRange(first);
        builder.AddRange(second);
        builder.AddRange(third);
        builder.AddRange(fourth);
        ...
        where K : notnull
    {
        ...
        var accumulator = new Dictionary<K, ImmutableArray<T>.Builder>(items.Length, comparer);
        for (int i = 0; i < items.Length; i++)
        {
            var item = items[i];
            ...
            {
                bucket = ImmutableArray.CreateBuilder<T>();
        ...
        {
            dictionary.Add(pair.Key, pair.Value.ToImmutable());
        }
